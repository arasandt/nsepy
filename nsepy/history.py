...
def get_history_quanta(**kwargs):
    ...
    print(url)
    df = url_to_df(
        url=url, params=params, schema=schema, headers=headers, scaling=scaling
    )
    return df
...
def url_to_df(url, params, schema, headers, scaling={}):
    ...
    tp = ParseTables(soup=bs, schema=schema, headers=headers, index="Date")
    df = tp.get_df()
...
def validate_params(
    ...
    if (futures and not option_type) or (not futures and option_type):  # EXOR
        ...
        if option_type in ("CE", "PE", "CA", "PA"):
            if not isinstance(strike_price, int) and not isinstance(
                strike_price, float
            ):
                raise ValueError(
...
def get_index_pe_history_quanta(symbol, start, end):
    ...
    bs = BeautifulSoup(resp.text, "lxml")
    tp = ParseTables(
        soup=bs, schema=INDEX_PE_SCHEMA, headers=INDEX_PE_HEADERS, index="Date"
    )
    df = tp.get_df()
    return df
...
def get_rbi_ref_history_quanta(start, end):
    ...
    Returns:
        pandas.DataFrame : A pandas dataframe object
    """
    resp = rbi_rate_history_url(
        fromDate=start.strftime("%d-%m-%Y"), toDate=end.strftime("%d-%m-%Y")
    )

    bs = BeautifulSoup(resp.text, "lxml")
    tp = ParseTables(
        soup=bs, schema=RBI_REF_RATE_SCHEMA, headers=RBI_REF_RATE_HEADERS, index="Date"
    )
    df = tp.get_df()
    return df
